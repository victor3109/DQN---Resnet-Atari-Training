#Sin comendarios por razon que es el mismo que el de Pong, ver sintaxis de pong DQN y RESNET
import gym
import numpy as np
import torch
...
BASE_DIR = "qbert_base_dir"
os.makedirs(BASE_DIR, exist_ok=True)

CHECKPOINT_PATH = os.path.join(BASE_DIR, 'qbert_RESNET_checkpoint.pth')
BEST_CHECKPOINT_PATH = os.path.join(BASE_DIR, 'qbert_RESNET_best.pth')
METRICS_PATH = os.path.join(BASE_DIR, 'qbert_RESNET_metrics.csv')
...
EXPLORATION_BURST_EVERY = 50  # para explorar mas cada 50 episodios, para mejorar la exploracion
BURST_EPSILON = 0.3  # episilon en exploracion burst
REWARD_SCALE = 0.01
...
# --- Environment Setup ---
def make_env():
    env = gym.make('QbertNoFrameskip-v4')  # cambiar juego
...
class DQNAgent:
    def __init__(self, load_checkpoint=False):
        ...
        #se uso RMSprop en lugar de Adam para mejorar la estabilidad, como Qbert es un juego mas complejo
    ...
    def select_action(self, state):
        #checar si es el momento de una exploracion burst, si es asi cambiar epsilon a BURST_EPSILON
    ...
    def update_model(self):
        ...
        #normalizamos las recompensas para mejorar la estabilidad del entrenamient
        ...
        #escalamos la perdida por la recompensa normalizada
